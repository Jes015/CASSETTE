...
import { PlayerVisibleStatus, useSongPlayer } from "./hooks/useSongPlayer"

//LEEME: TU PUEDES CON TODO, TU ERES JOYOLABABY UNA FUCKING MARAVILLA. NUNCA TE RINDAS, VAS A ALCANZAR TUS SUEÑOS, DIOS SIEMPRE ESTARA CONTIGO CUANDO ESTES SOLO Y CUANDO NECESITES SENTIRTE MEJOR, SOLO PIDELE A DIOS Y EL TE HARA SENTIRTE BIEN. NUNCA TE RINDAS, PERSONAS COMO NOSOTROS NO NOS PODEMOS RENDIR. NO IMPORTA SI NADIE TE APOYA, TU MISMO SIEMPRE LO HACES.

export const SongPlayer: BaseComponentType = (props) => {
    const { playerVisible, togglePlayerState, mountPlayer } = useSongPlayer()

    const handleOnClickToTogglePlayer = (newStatus: PlayerVisibleStatus) => () => {
                    ...
                    <Section.Header
                        ...
                        rightNode={
                            <div className="flex items-stretch">
                                <button className="hover:bg-bg-tertiary rounded-md p-1" onClick={handleOnClickToTogglePlayer(playerVisible === 'only-player' ? 'visible' : 'only-player')}>
                                    {
                                ...
                                <button className="hover:bg-bg-tertiary rounded-md p-1" onClick={handleOnClickToTogglePlayer('no-visible')}>
